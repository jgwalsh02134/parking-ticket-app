...
export async function registerRoutes(
  ...
  app.post("/api/extract-ticket", async (req: Request, res: Response) => {
    try {
      ...
      const instruction = [
        "You are reading an image of a City of Albany, New York parking ticket — either a photo of the paper citation or a screenshot/PDF page of the citation-detail view from the City's online portal (albany.rmcpay.com).",
        "Extract EVERY field below from the image so the rest of the app does not have to ask the user to retype anything. Return ONLY a compact JSON object, no prose, with these keys:",
        '{ "ticket": string, "vdate": "YYYY-MM-DD", "vtime": string, "location": string, "plate": string, "plate_type": string, "reg_expiration": string, "state": string, "make": string, "model": string, "body_type": string, "violation": string, "meter_number": string, "officer_id": string, "days_hours": string, "amount": string, "status": string, "hearing_date": string, "deadline_date": string, "is_camera": boolean }',
        "Rules:",
        ...
        "- vtime: the time of the violation, including a.m./p.m. if shown.",
        "- location: street address or particular place where the ticket was issued.",
        "- plate / state: license plate designation and its state of registration.",
        "- plate_type: the plate type/class if shown (e.g. PAS, COM, OMT, APPORTIONED).",
        "- reg_expiration: the registration expiration date if shown.",
        "- make / model: vehicle make and model if shown.",
        "- body_type: the vehicle body type/class if shown (e.g. SUBN, 4DSD, MCY).",
        "- violation: the violation description / code text (include both the code and its plain-English description if shown).",
        "- meter_number: the meter or muni-meter number if shown.",
        "- officer_id: the issuing officer's badge/shield number or ID if shown. If only an illegible signature is present with no ID number, leave empty.",
        "- days_hours: the days and hours the parking rule is in effect, if shown (e.g. 'Mon-Fri 8am-6pm').",
        "- amount: the fine amount as shown (e.g. '$30').",
        "- status: the citation status if shown (e.g. 'Open', 'Unpaid', 'In hearing').",
        "- hearing_date / deadline_date: any hearing date or response/appeal deadline shown, in YYYY-MM-DD if possible.",
        "- is_camera: true if this is a red-light (RLC) or school-zone-speed (SZS) camera violation, else false.",
        "- CRITICAL: For any field you cannot actually read in the image, use an empty string (or false for is_camera). NEVER guess, infer, or invent a value. Only report what is visibly printed.",
